...
Status: M1 is implemented. M2 is the active next packet; M3 and M4 remain deferred until its save and reward boundaries are trustworthy.
...
Exactly one next implementation task: M2, unified site-clear summaries and atomic automatic checkpoints with exactly-once discovery credit.
