...
## SC-17 — three terminal pilgrimage sites
- Objective: carry one build through the Workshop, one established mid-site and one consequential terminal site, then finish with a three-site route history and a route-specific final memory.
- Stable IDs / map contract: `route.pale_archive` → `assignment.pale_archive` / `site.pale_archive` / `arena.pale_archive` / `objective.recover_archive_index` / `boss.archivist_prime` / `memory.borrowed_lens`; `route.red_foundry` → `assignment.red_foundry` / `site.red_foundry` / `arena.red_foundry` / `objective.vent_red_foundry` / `boss.red_cardinal` / `memory.borrowed_shell`; `route.null_assembly` → `assignment.null_assembly` / `site.null_assembly` / `arena.null_assembly` / `objective.disable_null_anchors` / `boss.null_auditor` / `memory.unwritten_instruction`. Brass offers Pale or Red; Rootworks offers Red or Null. Terminal road IDs are `road.archive.misfiled_gate`, `road.archive.margin_clerk`, `road.foundry.slag_bridge`, `road.foundry.red13_cart`, `road.null.inventory_arch`, and `road.null.nameless_bench`.
- Owner: chapter/boss/arena content owns graph edges, costs, objectives, wave profiles, phase contracts and memories; simulation owns route eligibility, travel, objective rules, boss effects, carryover, save state and Results. UI only renders available routes and authoritative objective/boss state.
- Files: `content/chapter/first_chapter.json`, `content/bosses/first_slice.json`, `content/progression/first_chapter.json`, three new arena JSON files, `game/simulation.gd`, `game/main.gd`, `scripts/validate_content.py`, `tests/test_chapter.gd`, policy runners, chapter capture, runtime/verification/test documentation.
- Preserve: Workshop → Brass/Rootworks opening, optional-repair main mode, existing mid-site objectives and bosses, automatic combat, four active slots plus reserve, two currencies, unevolved viability, site-relative cadence, site:wave metrics, v1/v2 save migration and presentation authority boundary.
- Acceptance: only graph-valid routes are offered and accepted; Red is reachable from either mid-site while Pale and Null respect their single parent; every terminal route owns risk text and encounter plus merchant/service road nodes with a free continuation; mid-site memory returns to route choice and grants enough road Scrap; builds and prior memories carry; terminal arrival clears transient state; Archive recovery is ordered, Foundry vents rotate their active work point, and Null anchors suppress weapons only while the Saint works; all three bosses execute distinct data-owned phase traces; save/restore and Results preserve route history, all completed sites, bosses and memories; natural policies finish all four graph paths without requiring an evolution.
- Evidence: Godot 4.5.1, 1280×800, seed 147 fixture captures for each terminal objective/boss state plus natural full-expedition matrices; fixtures are labelled and not human playtests.
- Limitation: the three terminal sites reuse procedural art and the existing enemy families; enjoyment, route preference and 1× reaction comfort remain human-testing questions.
- Exactly one next task: uncoached 1× comparison of all four route paths, measuring objective comprehension and preferred replay path.
