...
func _ready():
	...
	for kind in ["line", "cone", "shot", "tether", "orbit", "rail", "beam", "blast", "censer", "winch", "halo", "radial", "ashen_censer", "long_hand", "repair_halo", "funeral_shots", "sermon", "benediction", "consecrated", "parade", "lattice", "repair", "hurt", "relay_hurt", "pickup"]:
...
func synth(kind: String):
	...
	for i in range(count):
		...
		if kind in ["cone", "radial", "repair_halo", "consecrated", "parade"]: sample = (sample + sin(TAU * frequency * 2.73 * t) * 0.35) / 1.35
		if kind in ["line", "rail", "tether", "winch", "long_hand", "lattice"]: sample = sample * 0.55 + sin(i * 37.19) * 0.45 * exp(-t * 30)
