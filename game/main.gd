...
func _draw():
	...
	if debug_visible:
		text_at("BUILD 0.1.0 | Godot %s | 1280×800 | seed %d | tick %d | %s" % [Engine.get_version_info().string, seed_value, sim.state.get("tick", 0), "FIXTURE CAPTURE" if capture_dir != "" or fixture_label else "LIVE"], Vector2(28, 745), 11, GOLD)
...
func draw_results():
	...
	var summary = "%d enemies stopped     %d Scrap remaining" % [sim.state.kills, sim.state.scrap]
	if sim.state.chapter_complete: summary += "     " + sim.current_route().name
	elif sim.optional_mode(): summary += "     %d optional repairs" % sim.state.machines.filter(func(m): return m.complete).size()
	else: summary += "     %d%% relay repaired" % (sim.state.progress * 100 / sim.config.relay.required_ticks)
	text_at(summary, Vector2(220, 409), 17)
	...
	var final_copy = sim.current_route().memory.text if won and sim.state.chapter_complete else ("MEMORY 01 / Your arm remembers a waterworks. Your bell remembers a factory. Neither remembers being asked to become a weapon." if won else ("The workshop keeps your place. Try another build, make room to dodge, or repair a machine for a useful reward." if sim.optional_mode() else "The workshop keeps your place. Try a different relic, leave the work circle to intercept threats, or return sooner to mend the relay."))
	wrapped(final_copy, Vector2(220, 510), 820, 18, PAPER)
