...
- `python tests/test_slice_manifest.py`: thirty-one manifest and graph acceptance/rejection tests.
...
- `godot --headless --path . --script res://tests/test_weapon_presentation.gd`: presentation-clock, effect-lifetime, mount-selection, and simulation-immutability checks for Nailer/Mercy Rail and Bell/Great Toll.
...
P15 Evolution breadth: `test_evolutions.gd` proves all ten data-owned recipes, atomic rejection/consumption, eight geometry/control/objective/resource identities, multi-form coexistence and save restoration. `test_acquisition.gd` additionally assembles every recipe through public shop/evolve commands at ordinary income bands, verifies each generated catalyst path, and proves shared Blue Wire can be earned and consumed separately for Long Hand and Contrition Lattice. `run_evolution_playthroughs.gd` carries all ten forms through complete two-leg chapters from controlled starting builds. `capture_evolutions.gd` renders the ten-entry Ledger and three configured geometry comparisons in `artifacts/evolutions`; these fixtures are not natural acquisition or human readability evidence.

P16 Gift breadth: `test_gift_breadth.gd` covers pure Honest Scale purchase projections, exact Loose Spring timing and source identity, Choir Filter recovery/damage trade-off, Brass Fuse segment identity/cadence, sale cleanup, Results metrics and additive save migration. `run_gift_playthroughs.gd` carries all four new Gifts through complete two-leg chapters and requires each causal rule to activate. `capture_gift_breadth.gd` renders the seven-Gift shop and two combat/status fixtures in `artifacts/gift-breadth`; these configured states are not natural acquisition or human comprehension evidence.

P18 weapon animation foundation: `test_weapon_presentation.gd` proves deterministic presentation durations and phase progress without simulation mutation. `capture_weapon_animation.gd` renders Nailer, Mercy Rail, Bell, Great Toll, and overlap states in `artifacts/weapon-animation`; these are configured stills, not evidence of real-time feel.
