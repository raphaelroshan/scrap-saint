...
func run_checks():
	...
	finish_travel(game)
	check(game.sim.state.phase == "arrival" and game.sim.state.site_id == "site.rootworks_pump", "second road choice reaches the destination briefing")
	...
	game._physics_process(0.0)
	check(game.sim.state_hash() == arrival_hash, "reading arrival cannot tick combat or repeat recovery")
	game.act("begin_site")
	check(game.sim.state.phase == "combat" and game.save_store.load_dictionary(save_path) == game.sim.snapshot(), "begin-site command starts combat and checkpoints the handoff")
