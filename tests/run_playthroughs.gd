			...
			if optional or s.progress >= sim.config.relay.required_ticks:
				for enemy in s.enemies:
					if enemy.major:
						...
						break
			if optional and variant == 3 and not s.machines[0].complete and not s.enemies.any(func(enemy): return enemy.major):
