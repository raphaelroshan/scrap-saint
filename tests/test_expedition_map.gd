...
func _initialize():
	...
	var map_read_hash = sim.state_hash()
	sim.step(Vector2.RIGHT)
	check(sim.state_hash() == map_read_hash, "reading the route map cannot advance simulation state")
	...
	check(sim.chapter.expedition_map.origin_preview.optional_preview.begins_with("Optional") and sim.chapter.routes.all(func(route): return str(route.optional_preview).begins_with("Optional")), "every level preview states that authored work is optional")
	check(sim.chapter.routes.all(func(route): return str(route.threat_preview) != "" and int(route.wave_count) > 0 and float(route.arrival_repair_floor) > 0.0), "every destination exposes threat, duration and arrival-floor preview data")
	...
	var committed_route = sim.snapshot()
	check(sim.command("choose_route", "route.brass_choir") == "OUTSIDE_WINDOW" and sim.snapshot() == committed_route, "Travel commits its fare and route exactly once")
