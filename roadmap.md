...
**Current user decisions (P14) supersede conflicting earlier requirements below:** free movement with optional repairs is the main game; relay defence remains a development comparison. Manual Foreman interruption was removed. The workshop has a following camera and minimap, ten base weapons, six ordinary enemy families, three run-local Gifts and two visible Evolutions. Mercy Rail and The Great Toll are optional, no Confluence is enabled, and encounters must support unevolved builds. Runtime cadence is eight 70-second waves with seven shops. The detailed milestone plan below is retained as the original design baseline, not a claim that every planned mechanic is implemented.
...
The next task is **uncoached 1x comparison of close-control, priority-control and repair-roaming builds**. P14 now enables Expansion A, The Great Toll and three Gifts without enabling Confluences. Measure whether players identify each role and trade-off, whether the larger shop pool creates dead choices, and whether the new effects remain legible in motion. Use [implementation packets](docs/implementation_packets.md) and [runtime status](docs/runtime_status.md) to distinguish implemented subsets from the original milestone specifications.
