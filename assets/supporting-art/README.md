...
After checking these assets in actual combat, extend the same material language to Rust Pilgrim (repair-tank silhouette), Forklift Brute (wide fork silhouette), Cinder Spitter (furnace mouth) and the Memory Crane elite. Then cover the remaining seven catalysts and five Gifts using their existing content IDs. Bosses, statuses, projectiles, route emblems and alternate poses need separate scoped passes. Do not mark the whole art catalogue complete from this first pack.
